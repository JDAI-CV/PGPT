...
def computeMetrics(gtFramesAll, motAll, outputDir, bSaveAll, bSaveSeq):
    ...
    metricsFinNames = ['mota','motp','pre','rec','miss_rate','false_rate','switches_rate']
    ...
    for si in range(nSeq):
        ...
        # iterate over frames
        #with open('seqName_{}.json'.format(seqName)) as f:
        data_dict = dict()
        data_dict[seqName] = []
        ...
        #        print(accAll)
                data_dict[seqName].append(accAll)
        #with open('seqName_{}.json'.format(seqName),'w') as f:
        ...
        if (bSaveSeq):
            ...
            for i in range(nJoints):
                ...
                metricsSeqAll[si]['miss_rate'][0,i] = 1.*metricsMid['num_misses']/numObj
                metricsSeqAll[si]['false_rate'][0,i] = 1.*numFP/numObj
                metricsSeqAll[si]['switches_rate'][0,i] = 1.*metricsMid['num_switches']/numObj
            ...
            idxs = np.argwhere(~np.isnan(metricsSeqAll[si]['miss_rate'][0,:nJoints]))
            metricsSeqAll[si]['miss_rate'][0,nJoints] = metricsSeqAll[si]['miss_rate'][0,idxs].mean()
            idxs = np.argwhere(~np.isnan(metricsSeqAll[si]['false_rate'][0,:nJoints]))
            metricsSeqAll[si]['false_rate'][0,nJoints] = metricsSeqAll[si]['false_rate'][0,idxs].mean()
            idxs = np.argwhere(~np.isnan(metricsSeqAll[si]['switches_rate'][0,:nJoints]))
            metricsSeqAll[si]['switches_rate'][0,nJoints] = metricsSeqAll[si]['switches_rate'][0,idxs].mean()
            ...
            metricsSeq['miss_rate'] = metricsSeq['miss_rate'].flatten().tolist()
            metricsSeq['false_rate'] = metricsSeq['false_rate'].flatten().tolist()
            metricsSeq['switches_rate'] = metricsSeq['switches_rate'].flatten().tolist()
    ...
    for i in range(nJoints):
        ...
        metricsFinAll['miss_rate'][0,i] = metricsMidAll['num_misses'][0,i] / numObj
        metricsFinAll['false_rate'][0,i] = numFP / numObj
        metricsFinAll['switches_rate'][0,i] = metricsMidAll['num_switches'][0,i] / numObj
    ...
    idxs = np.argwhere(~np.isnan(metricsFinAll['miss_rate'][0,:nJoints]))
    metricsFinAll['miss_rate'][0,nJoints] = metricsFinAll['miss_rate'][0,idxs].mean()
    idxs = np.argwhere(~np.isnan(metricsFinAll['false_rate'][0,:nJoints]))
    metricsFinAll['false_rate'][0,nJoints] = metricsFinAll['false_rate'][0,idxs].mean()
    idxs = np.argwhere(~np.isnan(metricsFinAll['switches_rate'][0,:nJoints]))
    metricsFinAll['switches_rate'][0,nJoints] = metricsFinAll['switches_rate'][0,idxs].mean()
    ...
    if (bSaveAll):
        ...
        metricsFin['miss_rate'] = metricsFin['miss_rate'].flatten().tolist()
        metricsFin['false_rate'] = metricsFin['false_rate'].flatten().tolist()
        metricsFin['switches_rate'] = metricsFin['switches_rate'].flatten().tolist()
