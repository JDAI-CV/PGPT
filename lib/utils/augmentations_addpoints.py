...
class Compose(object):
    ...
    def __call__(self, img_pre, img_next, boxes_pre=None, boxes_next=None, labels=None, points_pre=None, points_next=None):
        for t in self.transforms:
            img_pre, img_next, boxes_pre, boxes_next, labels, points_pre, points_next = \
                t(img_pre, img_next, boxes_pre, boxes_next, labels, points_pre, points_next)
        return img_pre, img_next, boxes_pre, boxes_next, labels, points_pre, points_next
...
class ConvertFromInts(object):
    def __call__(self, img_pre, img_next,
                 boxes_pre=None, boxes_next=None, labels=None, points_pre=None, points_next=None):
        return img_pre.astype(np.float32), img_next.astype(np.float32), \
               boxes_pre, boxes_next, labels, points_pre, points_next
...
class SubtractMeans(object):
    ...
    def __call__(self, img_pre, img_next, boxes_pre=None, boxes_next=None, labels=None, points_pre=None, points_next=None):
        ...
        img_next -= self.mean
        return img_pre.astype(np.float32), img_next.astype(np.float32), boxes_pre, boxes_next, labels, points_pre, points_next
...
class ResizeShuffleBoxes(object):
    ...
    def __call__(self, img_pre, img_next, boxes_pre=None, boxes_next=None, labels=None, points_pre=None, points_next=None):
        ...
        points_pre = points_pre[indexes_pre, :]
        points_next = points_next[indexes_next, :]
...
class ToTensor(object):
    def __call__(self,img_pre, img_next, boxes_pre=None, boxes_next=None, labels=None, points_pre=None, points_next=None):
        ...
        points_pre[0] = torch.from_numpy(points_pre[0].astype(float))
        points_pre[1] = torch.from_numpy(points_pre[1].astype(np.uint8))

        points_next[0] = torch.from_numpy(points_next[0].astype(float))
        points_next[1] = torch.from_numpy(points_next[1].astype(np.uint8))
...
class SSJAugmentation(object):
    ...
    def __call__(self, img_pre, img_next, boxes_pre, boxes_next, labels, points_pre, points_next):
        return self.augment(img_pre, img_next, boxes_pre, boxes_next, labels, points_pre, points_next)
...
def collate_fn(batch):
    ...
    for sample in batch:
        ...
        points_pre.append(sample[5][0].float())
        points_next.append(sample[6][0].float())
    ...
    return torch.stack(img_pre, 0), torch.stack(img_next, 0), \
           ...
           torch.stack(points_pre, 0), torch.stack(points_next, 0), \
