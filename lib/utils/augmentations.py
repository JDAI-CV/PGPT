...
class RandomMirror(object):
    ...
    def __call__(self, img_pre, img_next, boxes_pre=None, boxes_next=None, labels=None):
        ...
        if not random.randint(1,2):
            res_pre = self.mirror(img_pre, boxes_pre)
            res_next = self.mirror(img_next, boxes_next)
            img_pre= res_pre[0]
            img_next = res_next[0]

            img_pre = img_pre*1000   # add by rwj to generate judge outside
            img_next = img_next*1000 # add by rwj to generate judge outside
...
class FormatBoxes(object):
    ...
    '''
    def __init__(self, keep_box=True):
    ...
    def __call__(self,img_pre, img_next, boxes_pre=None, boxes_next=None, labels=None):
        ...
        if not self.keep_box:
            ...
            f = lambda boxes: np.expand_dims(
                np.expand_dims((boxes[:, :2] + boxes[:, 2:]) - 1, axis=1),
                axis=1
            )
        else:
            f = lambda boxes: np.expand_dims(
...
def collate_fn(batch):
    ...
    for sample in batch:
        origin_sample = sample[0]
        pose_sample = sample[1:]

        img_pre.append(origin_sample[0])
        img_next.append(origin_sample[1])

        boxes_pre.append(origin_sample[2][0].float())
        boxes_next.append(origin_sample[3][0].float())

        indexes_pre.append(origin_sample[2][1].byte())
        indexes_next.append(origin_sample[3][1].byte())

        labels.append(origin_sample[4].float())

        points_pre.append(pose_sample[0].float())
        points_next.append(pose_sample[1].float())
